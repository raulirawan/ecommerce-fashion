    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <a href="{{ route('produk.detail', $item->slug) }}" class="product-item md-height bg-gray d-block">
          <img src="{{ url($item->produk->first()->gambar ?? '') }}" alt="Image" class="img-fluid">
        </a>
        <h2 class="item-title"><a href="#">{{ $item->produk->first()->nama_produk ?? '' }}</a></h2>
        <strong class="item-price">{{ number_format($item->produk->first()->harga ?? 0) }}</strong>
      </div>
        @empty
